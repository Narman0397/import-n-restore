...
const TargetType = z.enum(["permohonan", "dataset_submission", "aset", "form_submission", "profile"]);
  ...
  .handler(async ({ data }) => {
    ...
    if (actorIds.length) {
      const { data: profs } = await supabaseAdmin
        .from("profiles").select("id, nama_lengkap").in("id", actorIds);
      actors = Object.fromEntries((profs ?? []).map((p) => [p.id, { nama: p.nama_lengkap }]));
    }
    return { rows: (rows ?? []).map((r) => ({ ...r, actor_nama: r.actor_id ? actors[r.actor_id]?.nama ?? null : null })) };
  });
